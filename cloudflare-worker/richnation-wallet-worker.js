...
// out of public static HTML: it holds a trusted credential (a Realtime
// Database secret, see below) that Realtime Database Rules unconditionally
// bypass, so once the matching rule change (see firebase-database.rules.json)
// is published, ONLY this Worker can change these specific fields, no
// matter what a browser sends Firebase directly. Every action below either
// verifies a real record (a real order, a real Paystack transaction) before
// crediting, or requires a secret only the admin panel knows before
// adjusting someone's money on request.
//
// ── HOW TO DEPLOY (no command line needed) ──
// 1. Firebase Console -> your project -> gear icon -> Project settings ->
//    Service accounts tab -> scroll down to "Database secrets" -> click
//    "Show" next to the existing secret (or "Add secret" if there isn't
//    one yet). This is a long random string, NOT the service account JSON
//    file used by richnation-push-worker.js, a completely different kind
//    of credential specific to Realtime Database's REST API. KEEP IT
//    PRIVATE, same as any password.
...
//    - Secret FIREBASE_DB_SECRET: paste the database secret from step 1.
...
// PARAMETER, see https://firebase.google.com/docs/database/rest/auth. This
// Worker used to generate a Google OAuth2 access token from a service
// account for that parameter, on the assumption (backed by Firebase's own
// docs on server-side access) that such a token bypasses Realtime Database
// Rules the same way the Admin SDK does. In practice, once real .validate
// rules were published, that token's writes kept getting rejected by
// exactly the rules it was supposed to be exempt from — the OAuth2 route
// evidently doesn't grant the bypass reliably here, whatever the scope
// combination. A Realtime Database SECRET (a legacy per-project credential,
// found under Project Settings -> Service accounts -> Database secrets,
// unrelated to the service-account JSON file richnation-push-worker.js
// uses) is Firebase's original, unambiguous, unconditional bypass
// mechanism for the REST API specifically, so this Worker uses that
// instead — simpler (no JWT signing needed) and does not depend on getting
// an OAuth scope combination exactly right.
function authedUrl(dbUrl, path, dbSecret){
  return dbUrl.replace(/\/+$/,'') + '/' + path + '.json?auth=' + dbSecret;
}
async function dbGet(dbUrl, path, dbSecret){
  const r = await fetch(authedUrl(dbUrl, path, dbSecret));
  if (!r.ok) return null;
  return await r.json();
}
...
async function dbPatch(dbUrl, path, data, dbSecret){
  const r = await fetch(authedUrl(dbUrl, path, dbSecret), {
    method:'PATCH', headers:{'Content-Type':'application/json'}, body: JSON.stringify(data)
  });
  return {ok:r.ok, status:r.status, text: r.ok ? '' : await r.text()};
}
async function dbPut(dbUrl, path, data, dbSecret){
  const r = await fetch(authedUrl(dbUrl, path, dbSecret), {
    method:'PUT', headers:{'Content-Type':'application/json'}, body: JSON.stringify(data)
  });
  return {ok:r.ok, status:r.status, text: r.ok ? '' : await r.text()};
}
async function dbPost(dbUrl, path, data, dbSecret){
  const r = await fetch(authedUrl(dbUrl, path, dbSecret), {
...
export default {
  async fetch(request, env){
    ...
    if (action === 'spend') {
      ...
      const current = (await dbGet(payload.dbUrl, 'rn_mall_customers/'+customerId+'/'+field, dbSecret)) || 0;
      ...
      const res1 = await dbPatch(payload.dbUrl, 'rn_mall_customers/'+customerId, update, dbSecret);
      if (!res1.ok) return json({error:'Could not update balance ('+dbErr(res1)+')'},502);
      if (field === 'walletBalance') {
        await dbPost(payload.dbUrl, 'rn_mall_wallet_transactions/'+customerId, {type:'debit',amount:amount,description:description||('Order payment'+(orderId?' ('+orderId+')':'')),date:new Date().toISOString().slice(0,10),ts:Date.now()}, dbSecret);
      }
    ...
    if (action === 'earn') {
      ...
      const settings = await dbGet(payload.dbUrl, 'rn_mall_settings', dbSecret) || {};
      const cust = await dbGet(payload.dbUrl, 'rn_mall_customers/'+customerId, dbSecret);
      ...
      if (reason === 'purchase') {
        ...
        const order = await dbGet(payload.dbUrl, 'rn_mall_orders/'+orderId, dbSecret);
        ...
        if (amount <= 0) return json({ok:true, amount:0});
        const newVal = (cust.points||0) + amount;
        const res1 = await dbPatch(payload.dbUrl, 'rn_mall_customers/'+customerId, {points:newVal}, dbSecret);
        if (!res1.ok) return json({error:'Could not credit purchase points ('+dbErr(res1)+')'},502);
        const res2 = await dbPatch(payload.dbUrl, 'rn_mall_orders/'+orderId, {pointsAwarded:true}, dbSecret);
      ...
      if (reason === 'referral') {
        ...
        const order = await dbGet(payload.dbUrl, 'rn_mall_orders/'+orderId, dbSecret);
        const referred = order && order.customer ? await dbGet(payload.dbUrl, 'rn_mall_customers/'+order.customer.id, dbSecret) : null;
        ...
        const priorOrders = await dbGet(payload.dbUrl, 'rn_mall_customer_orders/'+order.customer.id, dbSecret) || {};
        ...
        const res2 = await dbPatch(payload.dbUrl, 'rn_mall_orders/'+orderId, {referralAwarded:true}, dbSecret);
        if (!res2.ok) return json({error:'Credited points but could not mark order ('+dbErr(res2)+')'},502);
        return json({ok:true, amount:amount, newBalance:newVal});
      }
      ...
      if (reason === 'review') {
        ...
        const review = await dbGet(payload.dbUrl, 'rn_mall_reviews/'+productId+'/'+reviewKey, dbSecret);
        ...
        const res2 = await dbPatch(payload.dbUrl, 'rn_mall_reviews/'+productId+'/'+reviewKey, {pointsAwarded:true}, dbSecret);
    ...
    if (action === 'verify-topup') {
      ...
      const already = await dbGet(payload.dbUrl, 'rn_mall_processed_topups/'+reference, dbSecret);
      ...
      const cust2 = await dbGet(payload.dbUrl, 'rn_mall_customers/'+customerId, dbSecret);
      if (!cust2) return json({error:'Customer not found'},404);
      const newBalance = (cust2.walletBalance||0) + amount;
      const res1 = await dbPatch(payload.dbUrl, 'rn_mall_customers/'+customerId, {walletBalance:newBalance}, dbSecret);
      if (!res1.ok) return json({error:'Could not credit wallet ('+dbErr(res1)+')'},502);
      await dbPut(payload.dbUrl, 'rn_mall_processed_topups/'+reference, {customerId:customerId, amount:amount, ts:Date.now()}, dbSecret);
      await dbPost(payload.dbUrl, 'rn_mall_wallet_transactions/'+customerId, {type:'credit',amount:amount,description:'Card top-up (Ref: '+reference+')',date:new Date().toISOString().slice(0,10),ts:Date.now()}, dbSecret);
    ...
    if (action === 'refund-order') {
      ...
      const res2 = await dbPatch(payload.dbUrl, 'rn_mall_orders/'+orderId, {refunded:true}, dbSecret);
      if (!res2.ok) return json({error:'Refunded but could not mark order ('+dbErr(res2)+')'},502);
      await dbPost(payload.dbUrl, 'rn_mall_wallet_transactions/'+customerId, {type:'refund',amount:amount,description:'Refund for cancelled order '+orderId,date:new Date().toISOString().slice(0,10),ts:Date.now()}, dbSecret);
      return json({ok:true, amount:amount, newBalance:newBalance});
    }
    ...
    if (action === 'verify-payment') {
      ...
      const already = await dbGet(payload.dbUrl, 'rn_mall_processed_payments/'+reference, dbSecret);
      ...
      await dbPut(payload.dbUrl, 'rn_mall_processed_payments/'+reference, {amount:paidAmount, ts:Date.now()}, dbSecret);
    ...
    if (action === 'adjust') {
      ...
      let newVal = value;
      if (mode === 'add') {
        const current = (await dbGet(payload.dbUrl, collection+'/'+id+'/'+field, dbSecret)) || 0;
        newVal = current + (parseFloat(value)||0);
      }
      const update = {}; update[field] = newVal;
      const res1 = await dbPatch(payload.dbUrl, collection+'/'+id, update, dbSecret);
